...
export default function LogInForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      router.push("/main/home"); // go to home after login
    } catch (err: any) {
      console.error(err);
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md rounded-none bg-white/40 p-4 shadow-lg backdrop-blur-md md:rounded-2xl md:p-8 dark:bg-black/40">
        <h2 className="text-xl font-bold text-neutral-800 dark:text-neutral-200">
          Welcome back to <span style={{ color: "var(--color-primary)" }}>SafeSkin</span>
        </h2>
        <p className="mt-2 max-w-sm text-sm text-neutral-600 dark:text-neutral-300">
          Log in to view your history and run new scans.
        </p>
